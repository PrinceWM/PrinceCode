...
#include "detectspeedclient/detect.h"
...
timer_t g_detect_process_timeid = NULL;
...
#endif
{
    ...
    while(true)
    {
        ...
        if(pInfo->recvdFilesNumber >= FTDS_START_NUMBER && pInfo->fileIndex > 0)
        {
    ...
    while(!pInfo->EXITftds)
    {
        ...
        else if(feof(fp))
        {
            ...
            if(pInfo->channel->needToRestartConsumer)
            {
                ...
                while(true)
                {
                    if(pInfo->recvdFilesNumber >= FTDS_START_NUMBER)
                    {
                        time(&tmpT);
...
#endif
{
    ...
    while(!pInfo->EXITconsumer)
    {
        ...
        {
			pInfo->recvdFilesNumber = 0;//2013-1112
...
#endif
{
    ...
    {
		int sendNumber = 0;
        if(_GetTickCount64() - tmpTick > RESTART_CHANNEL_CONDITION_1*1000/2)//last 10 second recv no nexttrans
        {
    ...
    for(UINT i = 0; i<pSupplierTask->consumerDeque.size(); i++)
    {
        if(pSupplierTask->consumerDeque[i].UID == pConsumer->UID)
        {
            messager(msgERROR, "ERASE consumer UID %d, IP %s from channel %d supplier consumerDeque",
                     ...
                     pSupplierTask->channelID);
			pSupplierTask->consumerDeque[i].UID = 0;
            break;
        }
    }
    //pSupplierTask->lockDeleteQuitConsumer.unlock();
	//messager(msgERROR, "SET conumser UID %d, IP %s to 0",
	//		pConsumer->UID,
	//		pConsumer->ipAddr.sin_addr);
...
int addOneChannel2ConsumerTask(pduGetChannel *ppGet)
{
    ...
    for(cIter = globalTsConsumerTaskList.begin(); cIter != globalTsConsumerTaskList.end(); cIter ++)
    {
        ...
        {
            messager(msgINFO, "channel %d is in consumer task list, add ftds only", ppGet->channelID);
            ftds ftdsItem;
            ftdsItem.portFTDS = 0;
...
#endif
{
    ...
    while (1)
    {
		...
		messager(msgINFO, "get speed event 0");
        if(len >= sizeof(hdr))
        {
            hdr = *((pduHeader *)recvBuf);
			messager(msgINFO, "get speed event 1");
            if(len != hdr.Length)
            {
            ...
            {
				messager(msgINFO, "get speed event 2 type=0x%x ",hdr.Type);
                switch(hdr.Type)
                {
                case GET_CHANNEL:
                    {
                ...
                case NEXT_TRANS:
                    {
                        ...
                        for(sIter = globalTsSupplierTaskList.begin(); sIter != globalTsSupplierTaskList.end(); sIter ++)
                        {
                            if((*sIter).channelID == pNextTrans.channelID)
                            {
								...
								std::deque<tsConsumer>::iterator tcIter;
								for(tcIter = sIter->consumerDeque.begin(); tcIter != sIter->consumerDeque.end(); tcIter ++)
								{
									if(tcIter->UID == 0)
									{
										sIter->lockSupplierThread.lock();
										messager(msgERROR, "ERASE consumer UID %d, IP %s from channel %d supplier consumerDeque",
												 tcIter->UID,
												 inet_ntoa(tcIter->ipAddr.sin_addr),
												 sIter->channelID);										
										sIter->consumerDeque.erase(tcIter);
										sIter->lockSupplierThread.unlock();
										break;
									}
								}
				...
				case GET_SPEED:
					{						
						speed = (int *)(recvBuf + sizeof(pduHeader));
						fainalspeed = ntohl(*speed);
						messager(msgINFO, "get speed event fainalspeed = %d",fainalspeed);
						break;
					}
				...
				case DETECT_SPEED:
					{	
						...
						union sigval v;
						v.sival_ptr = g_detect_process_timeid;
						detectthread(v);
#elif defined(MOHO_WIN32)						
						detectthread(  //all consumer tasks use only one timer, maybe???
							NULL,        // handle to window for timer messages
							0,     // WM_TIMER message
							IDT_DETECT_PROCESS_TASK,     // timer identifier
...
int startdetectspeed()
{
	...
	pHeader.Type = DETECT_SPEED;
	pHeader.myUID = globalConfig.UID;
	pHeader.Length = sizeof(pHeader);

	memcpy(msg, &pHeader, sizeof(pHeader));
	len = sendto(sockPublicationSrv, msg, pHeader.Length, 0, (SOCKADDR *)(&localIP), tolen) ;
	if(len == SOCKET_ERROR)
	{
		messager(msgERROR, "send to failed, dest ip = %s, msgtype = %d, error number = %d", inet_ntoa(localIP.sin_addr), pHeader.Type, myGetLastError());
		ret = len;
	}
	else if(len != pHeader.Length)
	{
		messager(msgERROR, "send to error, dest ip = %s, msgtype = %d, msglen = %d, sendlen = %d", inet_ntoa(localIP.sin_addr), pHeader.Type, pHeader.Length, len);
		ret = len;
	}
	else
	{
		{
			messager(msgDEBUG, "send HOW_ARE_YOU to dest ip = %s:%d, seq = %d",
				inet_ntoa(localIP.sin_addr),
				ntohs(localIP.sin_port),
				globalConfig.seqNumber);
		}
	}
	return ret;
...
#endif
{
    ...
    //    for(UINT k = 0; k<sIter->consumerDeque.size(); k++)
    //    {
    //        if(sIter->consumerDeque[k].UID == 0)
    //        {
				//sIter->lockSupplierThread.lock();
				//sIter->consumerDeque[k].lockConsumerNextTransDeque.destroy();
				//sIter->consumerDeque.erase(sIter->consumerDeque.begin() + k);
				//messager(msgERROR, "ERASE consumer UID %d, IP %s from channel %d supplier consumerDeque",
				//		 sIter->consumerDeque[k].UID,
				//		 inet_ntoa(sIter->consumerDeque[k].ipAddr.sin_addr),
				//		 sIter->channelID);
				//sIter->lockSupplierThread.unlock();
				//break;
    //        }
...
int initTimer()
{
	...
	memset(&time_evp, 0, sizeof(struct sigevent));
	time_evp.sigev_value.sival_ptr = &g_detect_process_timeid;
	time_evp.sigev_notify = SIGEV_THREAD;
	time_evp.sigev_notify_function = detectthread;
	ret = timer_create(CLOCK_REALTIME, &time_evp, &g_detect_process_timeid);
	if(ret < 0)
	{
		messager(msgFATAL, "timer_create failed at taskNeighborNodeProcesser");
	}
	else
	{
		...
		ts.it_interval.tv_sec = TIME_PERIOD_OF_DETECT_TASK/1000;
		ts.it_interval.tv_nsec = (TIME_PERIOD_OF_DETECT_TASK%1000)*1000000;
		ts.it_value.tv_sec = TIME_PERIOD_OF_DETECT_TASK/1000;
		ts.it_value.tv_nsec = (TIME_PERIOD_OF_DETECT_TASK%1000)*1000000;

		ret = timer_settime(g_detect_process_timeid, TIMER_ABSTIME, &ts, NULL);
		if(ret < 0)
		{
			timer_delete(g_detect_process_timeid);
			messager(msgFATAL, "timer_settime failed at taskNeighborNodeProcesser");
		}
	}
    ...
    //timeGetDevCaps(&tc, c);
#if 0
	UINT ret = SetTimer(pMainWnd,IDT_DETECT_PROCESS_TASK,TIME_PERIOD_OF_DETECT_TASK,(TIMERPROC) detectthread);
	if(ret == 0)
	{
		messager(msgFATAL, "No timer is available at taskNeighborNodeProcesser");
	}
#endif
	//extern int startdetectspeed();
	startdetectspeed();

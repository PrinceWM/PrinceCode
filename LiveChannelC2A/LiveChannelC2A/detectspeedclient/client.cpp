...
client::client( SettingPara *inSettings ) 
{
	int rc = 0;
...
int client::setsendtoport(int port)
{
	if(port>0)
	{
...
int client::creatsock( ) 
{
	...
	int boolean = 1;
	len = sizeof(boolean);
	setsockopt( clientsock, SOL_SOCKET, SO_REUSEADDR, (char*) &boolean, len );
...
int client::run( int sessionid,int randid ) 
{
	...
	do {
		...
		if ( isudp ) 
		{
			...
			clientdgmbuff->udpid = htonl( sessionid );
			clientdgmbuff->randid = htonl( randid );
...
//#ifdef WIN32_BANDTEST
#if defined(MOHO_WIN32)
			/*at windows i just find ms delay function ,
			if need ns delay ,i use select*/
			if(delay>1000)
			{
...
//		printf("end %d delay=%d \n",packetTime.tv_sec,delay);
	...
	} while ( ! (exitflag  || (mEndTime.before( packetTime )))); 
	...
	while(1)
	{
		...
		else if(ret > 0)
		{
			if(FD_ISSET(clientsock, &fd_read))
			{
				...
				if(isudp)
				{
                                        ret = (int)recvfrom( clientsock, clientbuff, clientbufflen, 0,(struct sockaddr*) &sockadd, /*(int *)*/&sockaddlen );
				}
				else
				{				
...
{
	std::string filename=g_localPath;
	FILE* pFile = NULL;
#if defined(MOHO_X86)
	filename+="/";
#elif defined(MOHO_WIN32)
...
int client::checkconnectack(int *port,int *sessionid,int* randid)
{
	...
	int count = 0;
	int checktime = 0;
	struct sockaddr_in serveraddtmp;
#if defined(MOHO_WIN32)
	int serveraddtmplen = sizeof( struct sockaddr_in );
#elif defined(MOHO_X86)
        socklen_t serveraddtmplen = sizeof( struct sockaddr_in );
#endif
	...
	while ( count < 10 ) 
	{
		...
		else 
		{
			...
			{				
				...
				if(isudp)
				{
                                        rc = (int)recvfrom( clientsock, clientbuff, clientbufflen, 0,(struct sockaddr*) &serveraddtmp, /*(int *)*/&serveraddtmplen );
				}
				else
				{				
				...
				{//do udp ack
					...
					//port session id rand id
					*port = ntohl(((int*)(clientbuff))[0]);
					*sessionid = ntohl(((int*)(clientbuff))[1]);
					*randid = ntohl(((int*)(clientbuff))[2]);
					printf("recv randid %d\n",*randid);
					return 0;
				}
				else
				{//do tcp ack 					
...
#if defined(MOHO_WIN32)
        int tmpaddlen = sizeof( struct sockaddr_in );
#elif defined(MOHO_X86)
        socklen_t tmpaddlen = sizeof( struct sockaddr_in );
#endif

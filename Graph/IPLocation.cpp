...
int http_request(const char* url,std::vector<char>& content_buff, SOCKET* sock)
{
	...
	*sock = s;
	...
	do 
	{
		...
		if(!server)
		{
			printf("gethostbyname failed,host:%s\n",httpClient->host);
			ret =  -1;
			break;
		}

		printf("gethostbyname succes\n");
		sockaddr_in addr;
		...
		{
#ifdef WIN32
			printf("IPLocation  connect server,failed\n");
#else
			printf("IPLocation  connect server,failed,errno:%d\n",errno);
#endif
			ret =  -1;
			break;
		}
		...
		{
			printf("IPLocation  send request to server,failed\n");
			ret =  -1;
			break;
		}
		printf("send finish,len:%d\n",len);
		...
		printf( httpClient->content_data);
		printf("\n");
...
void copy_provice_data(char* dstbuff,char* proviceUtf8)
{
	...
	{ 
		printf("convert provice failed ,line:%d",__LINE__); 
	}
...
int sina_parsedata(const std::vector<char>& contentdata,IpQueryReplay* replay)
{
	...
	{
		printf("sina_parsedata ,data is invalid\n");
		return -1;
	}
	...
	if(!j_root)
	{
		printf("sina_parsedata j_root failed\n");
		return -1;
	}
	...
	{
		printf("sina_parsedata country:%s" ,j_country->valuestring);
	}
	...
	do{
		...
		{
			printf("sina_parsedata provice:%s" ,j_provice->valuestring);
			copy_provice_data(replay->provice,j_provice->valuestring);

		}
		else
		{
			printf("sina_parsedata  ,get province failed");
			ret = -1;
		}
		...
		{
			printf("sina_parsedata city:%s", j_city->valuestring);
			processCityData(replay->city,j_city->valuestring);
		}
		else
		{

			printf("sina_parsedata  ,get city failed");
			if(ret == -1)	//省市都没查到
			{
				break;
			}
		...
		{
			printf("sina_parsedata isp:%s", j_carrier->valuestring);
			processCarrierData(replay->carrir,j_carrier->valuestring);
		}
		else
		{
			printf("sina_parsedata  ,get carrier failed");
			ret = -1;
			break;
		}
...
int taobao_parsedata(const std::vector<char>& contentdata,IpQueryReplay* replay)
{
	...
	if(contentdata.size() <= 0 )
	{
		printf("taobao_parsedata ,data is invalid\n");
		return -1;
	}
	...
	if(!j_root)
	{
		printf("cJSON_Parse j_root failed\n");
		return -1;
	}
	...
	do
	{
		...
		if(!j_data)
		{
			printf("taobao_parsedata ,get j_data failed\n");
			ret = -1;
			break;
		}
		...
		if(j_provice && j_provice->valuestring
			&& strlen(j_provice->valuestring) > 0 )
		{
			printf("taobao_parsedata isp:%s", j_provice->valuestring);
			copy_provice_data(replay->provice,j_provice->valuestring);
		}
		else
		{
			printf("taobao_parsedata ,get region/provice failed\n");
			ret = -1;
		}
		...
		if(j_city && j_city->valuestring
			&& strlen(j_city->valuestring) > 0  )
		{
			printf("taobao_parsedata city:%s", j_city->valuestring);
			processCityData(replay->city,j_city->valuestring);
		}
		else
		{
			printf("taobao_parsedata ,get city  failed\n");		//省，城市都没取到
			if(ret == -1 )
			{
				break;
			}
		...
		if(j_carrier && j_carrier->valuestring
			&& strlen(j_carrier->valuestring) > 0 )
		{
			printf("taobao_parsedata isp:%s", j_carrier->valuestring);
			processCarrierData(replay->carrir,j_carrier->valuestring);
		}
		else
		{
			printf("taobao_parsedata ,get carrie  failed\n");		//省，城市都没取到
			ret = -1;			
			break;
...
int sohu_parsedata(const std::vector<char>& contentdata,IpQueryCxt* ctx)
{
	...
	if(contentdata.size() <= 0 )
	{
		printf("sohu_parsedata ,data is invalid\n");
		return -1;
	}
	...
	if(!j_root)
	{
		printf("sohu_parsedata  j_root failed\n");
		return -1;
	}
	...
	{
		printf("sohu_parsedata cip:%s", j_cip->valuestring);
		if(ctx->ip)
		{
			free(ctx->ip);
		}
	...
	{
		printf("sohu_parsedata ,get cip failed\n");
		return -1;
	}
...
void destroyQeuryCxt(IpQueryCxt* ctx)
{
	...
	{
		ctx->quit = 1;
		printf("destoryQueryCtx ctx->sock :%d\n",ctx->sock );
		if(ctx->threadid)
		{
#ifdef WIN32
			if(ctx->sock  != -1  )
